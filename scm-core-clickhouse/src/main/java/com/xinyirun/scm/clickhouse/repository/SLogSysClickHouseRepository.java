package com.xinyirun.scm.clickhouse.repository;
...
/**
 * 系统日志 ClickHouse Repository
 * 专门处理 s_log_sys 表的所有操作
 * 
 * @author SCM System
 * @since 1.0.39
 */
@Slf4j
@Repository
public class SLogSysClickHouseRepository {

    private static final DateTimeFormatter DATETIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
    private static final String TABLE_NAME = "s_log_sys";
     ...
     * 插入单条系统日志
     */
    public void insert(SLogSysClickHouseEntity sysLogEntity) {
        ...
        try {
            ...
            try (InsertResponse response = clickHouseClient
                    .insert(TABLE_NAME, List.of(sysLogEntity), insertSettings)
     ...
     * 批量插入系统日志 - 性能最优
     */
    public void batchInsert(List<SLogSysClickHouseEntity> sysLogEntities) {
        if (sysLogEntities == null || sysLogEntities.isEmpty()) {
            log.warn("批量插入系统日志数据为空，跳过操作");
            return;
        }

        long startTime = System.currentTimeMillis();
        int recordCount = sysLogEntities.size();
        
        try {
            // 设置创建时间
            LocalDateTime now = LocalDateTime.now();
            for (SLogSysClickHouseEntity entity : sysLogEntities) {
            ...
            // 执行批量插入
            try (InsertResponse response = clickHouseClient
                    .insert(TABLE_NAME, sysLogEntities, insertSettings)
    ...
    /**
     * 参数化分页查询系统日志 - 使用 ClickHouse Client v2 参数化查询
     * 支持用户名、用户名称、类型条件查询，支持分页和排序
     */
    public IPage<SLogSysClickHouseVo> selectPageWithParams(SLogSysClickHouseVo searchCondition) {
        try {
            ...
            // 2. 执行 COUNT 查询获取总数
            String countSql = "SELECT count(*) as total FROM s_log_sys " + whereClause;
            long total = 0;
            ...
            String dataSql = """
                SELECT id, type, user_name, staff_name, operation, time, class_name, 
                       class_method, http_method, params, session, url, ip, exception, 
                       c_time, request_id, result, tenant_code
                FROM s_log_sys 
            ...
            List<SLogSysClickHouseEntity> entities = new ArrayList<>();
            ...
            List<SLogSysClickHouseVo> records = entities.stream()
                .map(this::convertEntityToVo)
                .collect(Collectors.toList());
            
            IPage<SLogSysClickHouseVo> page = new Page<>(current, size, total);
            page.setRecords(records);
            
            log.info("参数化分页查询系统日志成功，当前页: {}, 页大小: {}, 总数: {}, 结果数量: {}", 
                    current, size, total, records.size());
            
            return page;
            
        } catch (Exception e) {
            log.error("参数化分页查询系统日志失败", e);
            throw new ClickHouseQueryException("参数化分页查询系统日志失败", "", e);
        }
    }
     ...
     * 转换 Entity 为 VO
     */
    private SLogSysClickHouseVo convertEntityToVo(SLogSysClickHouseEntity entity) {
    ...
    public SLogSysClickHouseVo getById(SLogSysClickHouseVo searchCondition) {
        ...
        String sql = """
            SELECT 
                id, type, user_name, staff_name, operation, time,
                class_name, class_method, http_method, params, session,
                url, ip, exception, c_time, request_id, result, tenant_code
            FROM s_log_sys 
            WHERE id = {id:String}
        ...
        try {
            ...
            if (!records.isEmpty()) {
                ...
                // 构建实体对象
                SLogSysClickHouseEntity entity = new SLogSysClickHouseEntity();
                ...
                entity.setParams(record.getString("params"));
                entity.setSession(record.getString("session"));
                ...
                entity.setResult(record.getString("result"));
                entity.setTenant_code(record.getString("tenant_code"));

                // 转换为VO并返回
                SLogSysClickHouseVo vo = convertEntityToVo(entity);
                log.info("根据ID查询系统日志成功，ID: {}, 租户: {}, 用户: {}", id, tenantCode, vo.getUser_name());
                return vo;
            }
            
            log.info("根据ID查询系统日志未找到记录，ID: {}, 租户: {}", id, tenantCode);
            return null;
            
        } catch (Exception e) {
            log.error("根据ID查询系统日志失败，ID: {}, 租户: {}", id, tenantCode, e);
            throw new ClickHouseQueryException("根据ID查询系统日志失败", sql, e);
        }
    }
     ...
     */
    private SLogSysClickHouseEntity mapReaderToEntity(ClickHouseBinaryFormatReader reader) {
        SLogSysClickHouseEntity entity = new SLogSysClickHouseEntity();
        ...
        entity.setParams(reader.getString("params"));
        entity.setSession(reader.getString("session"));
        ...
        entity.setResult(reader.getString("result"));
