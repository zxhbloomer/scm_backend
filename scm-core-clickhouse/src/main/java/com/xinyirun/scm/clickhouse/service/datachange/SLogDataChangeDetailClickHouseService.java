package com.xinyirun.scm.clickhouse.service.datachange;
...
import com.xinyirun.scm.clickhouse.repository.datachange.SLogDataChangeDetailClickHouseRepository;
...
public class SLogDataChangeDetailClickHouseService {
     ...
     */
    public SLogDataChangeOperateClickHouseVo findMainByOrderCode(String order_code) {
        try {
            // 1. 获取租户代码用于数据隔离
            ...
            if (changeEntities == null || changeEntities.isEmpty()) {
                ...
                return new SLogDataChangeOperateClickHouseVo();
            }
            ...
            List<SLogDataChangeDetailOldNewVo> changeVoList = changeEntities.stream()
                .flatMap(entity -> {
                    ...
                    return details.stream().map(detail -> {
                        SLogDataChangeDetailOldNewVo changeVo = new SLogDataChangeDetailOldNewVo();
            ...
            // 5. 设置变更列表并返回结果
            operateVo.setOldNewdetails(changeVoList);
