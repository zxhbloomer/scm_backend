...
import com.xinyirun.scm.bean.entity.business.so.settlement.BSoSettlementTotalEntity;
...
/**
 * 销售结算明细-数据汇总 Mapper 接口
 */
@Mapper
public interface BSoSettlementTotalMapper extends BaseMapper<BSoSettlementTotalEntity> {
     ...
     */
    @Select("""
            SELECT so_settlement_id 
            FROM b_so_settlement_detail_source_outbound 
            WHERE so_contract_id = #{contractId}
            """)
    List<Integer> selectSettlementIdsByContractId(@Param("contractId") Integer contractId);

    /**
     * 批量更新销售结算汇总数据
     * @param soSettlementIds 销售结算ID集合
     * @return 更新行数
     */
    @Update("""
            <script>
            UPDATE b_so_settlement_total t2
            JOIN (
                SELECT 
                    so_settlement_id,
                ...
                FROM b_so_settlement_detail_source_outbound
                WHERE so_settlement_id IN 
                <foreach collection="soSettlementIds" item="item" open="(" close=")" separator=",">
                    #{item}
                </foreach>
                GROUP BY so_settlement_id
            ) t1 ON t2.so_settlement_id = t1.so_settlement_id
            SET 
            ...
            WHERE t2.so_settlement_id IN 
            <foreach collection="soSettlementIds" item="item" open="(" close=")" separator=",">
                #{item}
            </foreach>
            </script>
            """)
    int batchUpdateSettlementTotal(@Param("soSettlementIds") LinkedHashSet<Integer> soSettlementIds);
    ...
    @Insert("""
            <script>
            INSERT INTO b_so_settlement_total (
                so_settlement_id
            )
            SELECT  
                so_settlement_id
            FROM b_so_settlement_detail_source_outbound t1
            WHERE t1.so_settlement_id IN (
                <foreach collection="soSettlementIds" item="item" separator=",">
                    #{item}
                </foreach>
            )
            AND NOT EXISTS (
                SELECT 1 FROM b_so_settlement_total t2 
                WHERE t2.so_settlement_id = t1.so_settlement_id
            )
            </script>
            """)
    int insertMissingSettlementTotal(@Param("soSettlementIds") LinkedHashSet<Integer> soSettlementIds);
