...
public interface BInPlanMapper extends BaseMapper<BInPlanEntity> {
    ...
    @Select("	<script>                                                                                                                                         "
            ...
            +"              'unit_id', tt1.unit_id,                                                                                                            "
            +"              'processing_qty', tt1.processing_qty,                                                                                              "
            +"              'processing_weight', tt1.processing_weight,                                                                                        "
            +"              'processing_volume', tt1.processing_volume,                                                                                        "
            +"              'unprocessed_qty', tt1.unprocessed_qty,                                                                                            "
            +"              'unprocessed_weight', tt1.unprocessed_weight,                                                                                      "
            +"              'unprocessed_volume', tt1.unprocessed_volume,                                                                                      "
            +"              'processed_qty', tt1.processed_qty,                                                                                                "
            +"              'processed_weight', tt1.processed_weight,                                                                                          "
            +"              'processed_volume', tt1.processed_volume,                                                                                          "
            +"              'remark', tt1.remark,                                                                                                              "
            +"              'goods_name', tt2.name,                                                                                                            "
            +"              'sku_name', tt2.spec,                                                                                                              "
            ...
            +"		LEFT JOIN s_dict_data  tab3 ON tab3.code = '"+ DictConstant.DICT_B_IN_PLAN_STATUS +"' AND tab3.dict_value = tab1.status                  "
            +"		LEFT JOIN s_dict_data  tab4 ON tab4.code = '"+ DictConstant.DICT_B_IN_PLAN_TYPE +"' AND tab4.dict_value = tab1.type                      "
    ...
    @Select("	SELECT                                                                                                                                          "
            ...
            +"		LEFT JOIN s_dict_data  tab6 ON tab6.code = '"+ DictConstant.DICT_B_IN_PLAN_STATUS +"' AND tab6.dict_value = tab1.status                  "
            +"		LEFT JOIN s_dict_data  tab7 ON tab7.code = '"+ DictConstant.DICT_B_IN_PLAN_TYPE +"' AND tab7.dict_value = tab1.type                      "
            +"		LEFT JOIN m_staff tab13 ON tab13.id = tab1.c_id                                                                                             "
            +"		LEFT JOIN m_staff tab14 ON tab14.id = tab1.u_id                                                                                             "
    ...
    @Select("	<script>                                                                                                                                           "
            +"	SELECT 	                                                                                                                                          "
            +"		SUM( IFNULL(tab2.processing_qty_total,0) )  as  processing_qty_total,                                                                      "       // 处理中数量
            +"		SUM( IFNULL(tab2.processing_weight_total,0) )  as  processing_weight_total,                                                                "       // 处理中重量
            +"		SUM( IFNULL(tab2.processing_volume_total,0) )  as  processing_volume_total,                                                                "       // 处理中体积
            +"		SUM( IFNULL(tab2.unprocessed_qty_total,0) )  as  unprocessed_qty_total,                                                                   "       // 待处理数量
            +"		SUM( IFNULL(tab2.unprocessed_weight_total,0) )  as  unprocessed_weight_total,                                                             "       // 待处理重量
            +"		SUM( IFNULL(tab2.unprocessed_volume_total,0) )  as  unprocessed_volume_total,                                                             "       // 待处理体积
            +"		SUM( IFNULL(tab2.processed_qty_total,0) )  as  processed_qty_total,                                                                       "       // 已处理数量
            +"		SUM( IFNULL(tab2.processed_weight_total,0) )  as  processed_weight_total,                                                                 "       // 已处理重量
            +"		SUM( IFNULL(tab2.processed_volume_total,0) )  as  processed_volume_total                                                                  "       // 已处理体积
            +"	FROM                                                                                                                                             "
            +"		b_in_plan tab1                                                                                                                              "
     ...
     * 校验计划编号是否重复
     */
    @Select( "select * from b_in_plan where true and is_del = false                           "
