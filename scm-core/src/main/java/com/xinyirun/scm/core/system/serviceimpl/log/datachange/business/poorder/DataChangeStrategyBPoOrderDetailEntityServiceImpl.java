...
@Slf4j
@Service
public class DataChangeStrategyBPoOrderDetailEntityServiceImpl extends LogChangeBaseServiceImpl implements IDataChangeStrategyService {
    ...
    @Autowired
    BPoOrderDetailMapper detailMapper;
    ...
    public SDataChangeLogDetailVo getDataChangeVoByInsert(BoundSql boundSql) {
        ...
        BPoOrderDetailEntity entity = (BPoOrderDetailEntity)parameterObject;
        ...
        vo.setEntity_name(BPoOrderDetailEntity.class.getName());
        // 根据 u_id 获取 u_name
    ...
    public SDataChangeLogDetailVo getDataChangeVoByUpdateBefore(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        // 查询更新前的数据
        BPoOrderDetailEntity condition = (BPoOrderDetailEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        BPoOrderDetailEntity entity = detailMapper.selectById(condition.getId());
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoOrderEntity masterEntity = poOrderMapper.selectById(entity.getPo_order_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
    ...
    public SDataChangeLogDetailVo getDataChangeVoByUpdateAfter(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        BPoOrderDetailEntity entity = (BPoOrderDetailEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoOrderEntity masterEntity = poOrderMapper.selectById(entity.getPo_order_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
    ...
    public SDataChangeLogDetailVo getDataChangeVoByDelete(BoundSql boundSql) {
        ...
        BPoOrderDetailEntity entity = detailMapper.selectById(id);
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoOrderEntity masterEntity = poOrderMapper.selectById(entity.getPo_order_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
    ...
    public String getOrderCode(Integer id) {
        log.debug("------开始 获取 poOrderDetailMapper code -----------");
        Integer masterId = detailMapper.selectById(id).getPo_order_id();
        BPoOrderEntity masterEntity = poOrderMapper.selectById(masterId);
        String code = masterEntity.getCode();
        log.debug("根据id取code：{}", code);
        log.debug("------结束 获取 poOrderDetailMapper code -----------");
        return code;
    }
    ...
    public SLogDataChangeDetailVo getCTimeExtension(String param, String _json_data, String clm_name, String clm_label) {
        ...
        if (!Objects.isNull(_json_data)) {
            ...
            try {
                BPoOrderDetailEntity entity = JSONObject.parseObject(_json_data, BPoOrderDetailEntity.class);
                BPoOrderEntity masterEntity = poOrderMapper.selectById(entity.getPo_order_id());
