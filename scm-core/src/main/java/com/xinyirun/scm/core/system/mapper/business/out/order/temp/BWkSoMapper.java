package com.xinyirun.scm.core.system.mapper.business.out.order.temp;
...
import com.xinyirun.scm.bean.entity.busniess.out.order.temp.BWkSoEntity;
 ...
 */
@Repository
public interface BWkSoMapper extends BaseMapper<BWkSoEntity> {
    ...
    int deleteB_wk_soWork00();

    /**
     * 删除数据临时表
     */
    @Delete(
            "       delete from b_wk_so_detail ;                                               "
    )
    int deleteB_wk_so_detail01();

    /**
     * 锁定临时表
     */
    @Select(
            "       select 1 from b_wk_so for update;                                               "
    )
    List<Integer> lockB_wk_so_detail10();

    /**
     * 锁定临时表
     */
    @Select(
            "       select 1 from b_wk_so_detail for update;                                               "
    )
    List<Integer> lockB_wk_so_detail11();
    ...
    @Select(""
        ...
        + "			WHEN ( t2.credit_no IS NULL AND t1.client_credit_no IS NOT NULL ) THEN '3'                          "
        ...
        + "		FROM                                                                                                    "
        + "			b_wk_so t1                                                                                          "
        + "			LEFT JOIN m_customer t2 ON t1.client_credit_no = t2.credit_no                                       "
    ...
    ApiBWkSoDetailVo checkB_wk_so_detail21();

    /**
     * 更新数据
     */
    @Update(""
            + "		INSERT INTO b_out_order (                                                                           "
            + "			order_no,                                                                                       "
            ...
            + "		FROM                                                                                                "
            + "			b_wk_so t1                                                                                      "
            + "			LEFT JOIN b_out_order t2 ON t1.order_no = t2.order_no                                           "
            + "			LEFT JOIN m_customer t3 ON t1.client_credit_no = t3.credit_no                                   "
    ...
    @Update(""
            + "	UPDATE                                                                                                  "
            + "		b_out_order t1                                                                                      "
            + "	INNER JOIN b_wk_so t2 ON t1.order_no = t2.order_no                                                      "
            + "	LEFT JOIN m_customer t3 ON t2.client_credit_no = t3.credit_no                                           "
            ...
            + "		t1.client_id = t3.id,                                                                               "
            + "		t1.client_code = t3.CODE,                                                                           "
            + "		t1.owner_id = t4.id,                                                                                "
            + "		t1.owner_code = t4.CODE,                                                                            "
            + "		t1.business_type_id = t5.id,                                                                        "
            + "		t1.business_type_code = t5.CODE,                                                                    "
    ...
    void updateB_wk_so30();

    /**
     * 插入明细数据
     */
    @Update(""
            + "			INSERT INTO b_out_order_goods (                                                                "
            + "				order_id,                                                                                   "
            + "				no,                                                                                         "
            + "				sku_id,                                                                                     "
            + "				sku_code,                                                                                   "
    ...
    @Update(""
            + "	DELETE                                                                                                  "
            + "	FROM                                                                                                    "
            + "		b_out_order_goods t1                                                                                "
            + "	WHERE                                                                                                   "
            + "		EXISTS ( SELECT 1 FROM b_out_order t2 INNER JOIN b_wk_so_detail t3                                  "
            + "		ON t2.order_no = t3.order_no WHERE t1.order_id = t2.id )                                            "
    )
    void deleteB_wk_so_detail32();
    ...
    @Update(""
            + "	UPDATE b_order t1                                                                                       "
            + "		INNER JOIN b_out_order t2 ON t1.serial_id = t2.id                                                   "
            + "			AND t1.serial_type = 'b_out_order'                                                              "
            + "	SET t1.serial_id = t2.id,                                                                               "
            + "		t1.serial_type = 'b_out_order',                                                                     "
            + "		t1.order_no = t2.order_no,                                                                          "
            + "		t1.contract_no = t2.contract_no,                                                                    "
            + "		t1.u_time = now( ),                                                                                 "
            + "		t1.bill_type = t2.bill_type,                                                                        "
            ...
            + "		t1.business_type_code = t2.business_type_code,                                                      "
            + "     t1.source_type = '"+ DictConstant.DICT_B_ORDER_SOURCE_TYPE_ERP +"',                                 "
            + "		t1.customer_id=t2.client_id                                                                         "
            + "	WHERE                                                                                                   "
            + "		t2.id = t1.serial_id                                                                                "
            + "		AND t1.serial_type = 'b_out_order'                                                                  "
    )
    void updateB_wk_so33();
}
