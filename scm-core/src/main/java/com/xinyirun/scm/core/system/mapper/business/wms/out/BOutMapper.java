...
public interface BOutMapper extends BaseMapper<BOutEntity> {
    ...
    @Select("""
            ...
            FROM
            	...
            	LEFT JOIN s_dict_data tab3 ON tab3.code = 'b_out_status' AND tab3.dict_value = tab1.status
              ...
              LEFT JOIN b_out_plan tab15 ON tab15.id = tab16.out_plan_id
            	WHERE TRUE
            	 ...
            	 AND (tab1.consignee_id = #{param.consignee_id} or #{param.consignee_id} is null)
            ...
            """)
    IPage<BOutVo> selectPage(Page<BOutVo> page, @Param("param") BOutVo searchCondition);
    ...
    @Select("""
            SELECT
            	...
            	tab13.name as c_name,
            	tab14.name as u_name,
            	tab15.type as so_contract_type,
            	tab16.label as so_contract_type_name,
            	tab15.seller_name,
            	tab15.payment_type as so_contract_payment_type,
            	tab17.label as payment_type_name,
            	tab15.settle_type as so_contract_settle_type,
            	tab18.label as settle_type_name,
            	tab15.delivery_date,
            	tab15.delivery_location,
            	tab19.status as so_order_status,
            	tab20.label as so_order_status_name,
            	tab19.delivery_type as so_order_delivery_type,
            ...
            FROM
            	...
            	LEFT JOIN s_dict_data tab3 ON tab3.code = 'b_out_status' AND tab3.dict_value = tab1.status
            	LEFT JOIN s_dict_data tab4 ON tab4.code = 'b_out_type' AND tab4.dict_value = tab1.type
            	LEFT JOIN b_out_attach tab5 ON tab5.out_id = tab1.id
            	LEFT JOIN b_so_contract tab15 ON tab15.id = tab1.contract_id
            	LEFT JOIN s_dict_data tab16 ON tab16.code = 'b_so_contract_type' AND tab16.dict_value = tab15.type
            	LEFT JOIN s_dict_data tab17 ON tab17.code = 'b_so_contract_payment_type' AND tab17.dict_value = tab15.payment_type
            	LEFT JOIN s_dict_data tab18 ON tab18.code = 'b_so_contract_settle_type' AND tab18.dict_value = tab15.settle_type
            	LEFT JOIN b_so_order tab19 ON tab19.id = tab1.order_id
            	LEFT JOIN s_dict_data tab20 ON tab20.code = 'b_so_order_status' AND tab20.dict_value = tab19.status
              ...
              LEFT JOIN b_out_plan tab22 ON tab22.id = tab23.out_plan_id
            	WHERE tab1.id = #{id}
            """)
    BOutVo selectById(@Param("id") Integer id);
    ...
    @Select("""
            ...
            """)
    List<BOutVo> selectExportList(@Param("param") BOutVo param);
    ...
    @Select("""
            ...
            """)
    BOutVo querySum(@Param("param") BOutVo searchCondition);
     ...
     */
    @Select("""
            select DISTINCT t.contract_id FROM b_out t where t.id = #{outboundId}
            """)
    List<Integer> selectContractIdsByOutboundId(Integer outboundId);
}