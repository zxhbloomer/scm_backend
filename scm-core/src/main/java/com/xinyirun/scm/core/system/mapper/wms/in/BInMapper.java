...
public interface BInMapper extends BaseMapper<BInEntity> {
    ...
    @Select("	SELECT                                                                                                                          "
            ...
            +"		LEFT JOIN s_dict_data tab16 ON tab16.code = '" + DictConstant.DICT_B_PO_CONTRACT_TYPE + "' AND tab16.dict_value = tab15.type                        "
            +"		LEFT JOIN s_dict_data tab17 ON tab17.code = '" + DictConstant.DICT_B_PO_CONTRACT_PAYMENT_TYPE + "' AND tab17.dict_value = tab15.payment_type      "
            +"		LEFT JOIN s_dict_data tab18 ON tab18.code = '" + DictConstant.DICT_B_PO_CONTRACT_SETTLE_TYPE + "' AND tab18.dict_value = tab15.settle_type        "
            +"		LEFT JOIN b_po_order tab19 ON tab19.id = tab1.order_id                                                                     "
            +"		LEFT JOIN s_dict_data tab20 ON tab20.code = '" + DictConstant.DICT_B_PO_ORDER_STATUS + "' AND tab20.dict_value = tab19.status                 "
            +"		LEFT JOIN s_dict_data tab21 ON tab21.code = '" + DictConstant.DICT_B_PO_ORDER_DELIVERY_TYPE + "' AND tab21.dict_value = tab19.delivery_type  "
            +"    LEFT JOIN m_staff tab13 ON tab13.id = tab1.c_id                                                                               "
            +"    LEFT JOIN m_staff tab14 ON tab14.id = tab1.u_id                                                                               "
