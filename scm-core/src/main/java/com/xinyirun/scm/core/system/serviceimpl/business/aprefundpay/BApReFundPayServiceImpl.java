...
import com.xinyirun.scm.bean.system.vo.business.aprefund.BApReFundPayAttachVo;
...
public class BApReFundPayServiceImpl extends ServiceImpl<BApRefundPayMapper, BApReFundPayEntity> implements IBApReFundPayService {
    ...
    public InsertResultAo<BApReFundPayVo> insert(BApReFundPayVo searchCondition) {
        ...
        bApReFundPayDetailEntity.setPo_code(searchCondition.getPo_code());
        bApReFundPayDetailEntity.setPo_contract_code(searchCondition.getPo_contract_code());
        ...
        SFileEntity sFileEntity = insertFile(fileEntity, searchCondition.getPush_files());
    ...
    public UpdateResultAo<BApReFundPayVo> paymentReview(BApReFundPayVo searchCondition) {
        ...
        if (bApReFundEntity.getRefund_amount().compareTo(bApReFundEntity.getRefunded_amount()) == 0){
            bApReFundEntity.setPay_status(DictConstant.DICT_B_AP_REFUND_PAY_STATUS_TWO);
        } else if (bApReFundEntity.getRefund_amount().compareTo(bApReFundEntity.getRefunded_amount()) > 0){
            bApReFundEntity.setPay_status(DictConstant.DICT_B_AP_REFUND_PAY_STATUS_ONE);
        } else if (bApReFundEntity.getRefund_amount().compareTo(bApReFundEntity.getRefunded_amount()) < 0){
            throw new BusinessException("付款复核，更新失败");
        }
    ...
    public UpdateResultAo<BApReFundPayVo> cancel(BApReFundPayVo searchCondition) {
        ...
        bApReFundEntity.setPay_status(CollectionUtil.isEmpty(bApReFundPayVos)?DictConstant.DICT_B_AP_REFUND_PAY_STATUS_ZERO:DictConstant.DICT_B_AP_REFUND_PAY_STATUS_ONE);
