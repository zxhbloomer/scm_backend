 ...
 */
@Repository
public interface SLogApiMapper extends BaseMapper<SLogApiEntity> {
    ...
    @Select("    "
            ...
            + "       ")
    IPage<SLogApiVo> selectPage(Page page, @Param("p1") SLogApiVo searchCondition);

    @Select("  select count(*) as c from (  "
            + " select t.id from s_log_api t                                                                                                       "
            + "   where true                                                                                                                    "
            + "      and (t.type like CONCAT ('%',#{p1.type,jdbcType=VARCHAR},'%') or #{p1.type,jdbcType=VARCHAR} is null)                      "
            + "      and (t.url like CONCAT ('%',#{p1.url,jdbcType=VARCHAR},'%') or #{p1.url,jdbcType=VARCHAR} is null)                         "
            + "      and (t.operation like CONCAT ('%',#{p1.operation,jdbcType=VARCHAR},'%') or #{p1.operation,jdbcType=VARCHAR} is null)       "
            ...
            + "       )  sub ")
    Integer getLimitCount(@Param("p1")  SLogApiVo searchCondition);
}
