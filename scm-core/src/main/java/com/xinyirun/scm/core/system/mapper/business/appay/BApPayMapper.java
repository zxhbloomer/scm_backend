...
public interface BApPayMapper extends BaseMapper<BApPayEntity> {
    ...
    @Select({
            ...
            "      LEFT JOIN s_dict_data tab4 ON tab4.CODE = '"+ DictConstant.DICT_B_AP_PAY_BILL_STATUS +"' AND tab4.dict_value = tab1.status ",
            "      LEFT JOIN s_dict_data tab5 ON tab5.CODE = '"+ DictConstant.DICT_B_AP_TYPE +"' AND tab5.dict_value = tab1.type ",
     ...
     */
    @Select("select * from b_ap_pay where ap_id = #{p1} and status = #{p2}  ")
     ...
     * 查询付款单明细
     */
    @Select("SELECT t1.*, t2.name, t2.bank_name, t2.account_number FROM b_ap_pay_detail t1 LEFT JOIN m_bank_accounts t2 ON t1.bank_accounts_id = t2.id WHERE t1.ap_pay_id = #{p1}")
     ...
     */    @Update({
        ...
        "    t1.cancel_amount_total = CASE WHEN t1.status = '2' THEN t2.cancel_amount_total ELSE 0 END ", // 作废
        "WHERE t1.ap_id IN ",
