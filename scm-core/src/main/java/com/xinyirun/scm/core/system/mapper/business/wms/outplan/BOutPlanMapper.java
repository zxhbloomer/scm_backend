...
import com.xinyirun.scm.core.system.config.mybatis.typehandlers.OutPlanDetailListTypeHandler;
...
public interface BOutPlanMapper extends BaseMapper<BOutPlanEntity> {
    ...
    @Select("""
            ...
            SELECT                                                                                                                                     
           	...
           	FROM                                                                                                                                            
           		...
           		LEFT JOIN m_enterprise t2 ON t2.id = tab1.consignee_id                                                                                     
           	    LEFT JOIN (SELECT tt1.out_plan_id, JSON_ARRAYAGG(                                                                                           
           	        JSON_OBJECT(                                                                                                                            
                     'id', tt1.id,                                                                                                                      
                     'code', tt1.code,                                                                                                                  
                     'no', tt1.no,                                                                                                                      
                     'out_plan_id', tt1.out_plan_id,                                                                                                      
           	     ...
           	     FROM b_out_plan_detail tt1 LEFT JOIN m_goods_spec tt2 ON tt1.goods_code = tt2.goods_code AND tt1.sku_code = tt2.code                     
                 LEFT JOIN b_so_order_total tt3 ON tt1.order_id = tt3.so_order_id                                                                      
             ...
             GROUP BY tt1.out_plan_id) tab2 ON tab1.id = tab2.out_plan_id                                                                                
           		LEFT JOIN s_dict_data  tab3 ON tab3.code = 'b_out_plan_status' AND tab3.dict_value = tab1.status                  
           		LEFT JOIN s_dict_data  tab4 ON tab4.code = 'b_out_plan_type' AND tab4.dict_value = tab1.type                      
           ...
           LEFT JOIN m_staff tab14 ON tab14.id = tab1.u_id                                                                                               
           		WHERE TRUE                                                                                                                                  
           		 ...
           		 AND (tab1.consignee_id = #{p1.consignee_id}  or #{p1.consignee_id} is null   )                                                            
            ...
            """)
    @Results({
            @Result(property = "detailListData", column = "detailListData", javaType = List.class, typeHandler = OutPlanDetailListTypeHandler.class),
    })
    IPage<BOutPlanVo> selectPage(Page<BOutPlanVo> page, @Param("p1") BOutPlanVo searchCondition);
    ...
    @Select("""
           	...
           	FROM                                                                                                                                            
                 ...
                 LEFT JOIN m_bin tt7 ON tt7.id = tt1.bin_id                                                                                            
             GROUP BY tt1.out_plan_id) tab2 ON tab1.id = tab2.out_plan_id                                                                                
           		LEFT JOIN b_out_plan_attach tab3 on tab1.id = tab3.out_plan_id                                                                               
           		LEFT JOIN s_dict_data  tab6 ON tab6.code = 'b_out_plan_status' AND tab6.dict_value = tab1.status                  
           		LEFT JOIN s_dict_data  tab7 ON tab7.code = 'b_out_plan_type' AND tab7.dict_value = tab1.type                      
           		LEFT JOIN m_staff tab13 ON tab13.id = tab1.c_id                                                                                             
           		LEFT JOIN m_staff tab14 ON tab14.id = tab1.u_id                                                                                             
    ...
    @Select("""
           	...
           	FROM                                                                                                                                             
           		b_out_plan tab1                                                                                                                              
           		LEFT JOIN b_out_plan_total tab2  ON tab1.id = tab2.out_plan_id                                                                               
           		WHERE TRUE                                                                                                                                   
           		 AND tab1.is_del = false                                                                                                                     
           		 AND (tab1.status = #{p1.status} or #{p1.status} is null or #{p1.status} = '')                                                               
           		 AND (tab1.code like CONCAT('%', #{p1.code}, '%') or #{p1.code} is null or #{p1.code} = '')                                               
            ...
            """)
    BOutPlanVo querySum(@Param("p1") BOutPlanVo searchCondition);
    ...
    @Select("""
            ...
            SELECT @row_num:= @row_num+ 1 as no,tb1.* from (                                                                                                  
               ...
               LEFT JOIN m_staff tab14 ON tab14.id = tab1.u_id                                                                                               
              	...
              	GROUP BY                                                                                                                                        
              		tab2.out_plan_id) as tb1,(select @row_num:=0) tb2                                                                                            
              		  </script>                                                                                                                                 
            """)
    @Results({
            @Result(property = "detailListData", column = "detailListData", javaType = List.class, typeHandler = OutPlanDetailListTypeHandler.class),
    })
    List<BOutPlanVo> selectExportList(@Param("p1")BOutPlanVo param);
            ...
            SELECT                                                                                                                                    
           	...
           	FROM                                                                                                                                            
           		...
           		WHERE TRUE                                                                                                                                  
           		 ...
           		 AND (tab1.code = #{p1.code} or #{p1.code} is null or #{p1.code} = '')                                  
            """)
    Long selectExportCount(@Param("p1")BOutPlanVo param);

    /**
     * 根据code查询出库计划
     */
    @Select("select * from b_out_plan where code = #{code} and is_del = false")
    BOutPlanVo selectByCode(@Param("code") String code);
    ...
    @Select("""
            SELECT                                                                             
              t1.so_contract_code AS contract_code,                                            
              t1.so_contract_id AS contract_id,                                                
              t1.id AS order_id,                                                               
            ...
            FROM                                                                               
              b_so_order t1                                                                    
              LEFT JOIN b_so_order_detail t2 ON t1.id = t2.so_order_id                        
              LEFT JOIN (                                                                      
                SELECT                                                                         
                  order_detail_id,                                                             
                  SUM(processed_qty) AS processed_qty,                                         
                  SUM(unprocessed_qty) AS unprocessed_qty                                      
                FROM b_out_plan_detail                                                          
            ...
            ORDER BY                                                                           
              ...
              t2.id                                                                            
            """)
    List<BOutPlanDetailVo> initPlanData(@Param("searchCondition") BOutPlanDetailVo searchCondition);
