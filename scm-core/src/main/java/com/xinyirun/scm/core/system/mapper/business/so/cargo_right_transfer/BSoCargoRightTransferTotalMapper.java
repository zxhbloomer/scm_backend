...
import com.xinyirun.scm.bean.entity.business.so.cargo_right_transfer.BSoCargoRightTransferTotalEntity;
 ...
 */
@Repository
public interface BSoCargoRightTransferTotalMapper extends BaseMapper<BSoCargoRightTransferTotalEntity> {
    ...
    @Select("""
            ...
            FROM b_so_cargo_right_transfer t
            WHERE t.so_contract_id = #{contractId}
              AND t.is_del = false
            """)
    ...
    @Insert("""
            <script>
            INSERT INTO b_so_cargo_right_transfer_total (cargo_right_transfer_id, so_order_id, cargo_right_untransfer_qty_total, cargo_right_transfering_qty_total, cargo_right_transferred_qty_total, cargo_right_transfer_cancel_qty_total)
            SELECT t.id, t.so_order_id, 0, 0, 0, 0
            FROM b_so_cargo_right_transfer t
            WHERE t.id IN
              ...
              AND NOT EXISTS (
                  SELECT 1 FROM b_so_cargo_right_transfer_total total
    ...
    @Update("""
            <script>
            UPDATE b_so_cargo_right_transfer_total total
            JOIN (
                SELECT 
                ...
                FROM b_so_cargo_right_transfer main
                LEFT JOIN b_so_cargo_right_transfer_detail detail ON main.id = detail.cargo_right_transfer_id
                WHERE main.id IN
                <foreach collection="cargoRightTransferIds" item="id" open="(" separator="," close=")">
                    #{id}
                </foreach>
