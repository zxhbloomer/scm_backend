...
@Slf4j
@Service
public class DataChangeStrategyMEnterpriseAttachEntityServiceImpl extends LogChangeBaseServiceImpl implements IDataChangeStrategyService {

    @Autowired
    MStaffMapper staffMapper;

    @Autowired
    MEnterpriseAttachMapper enterpriseAttachMapper;
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据新增的对象，还没有进行比对")
    public SDataChangeLogDetailVo getDataChangeVoByInsert(BoundSql boundSql) {
        ...
        MEnterpriseAttachEntity entity = (MEnterpriseAttachEntity)parameterObject;
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        vo.setOrder_code(enterpriseMapper.selectId(entity.getEnterprise_id()).getCode());
        ...
        vo.setEntity_name(MEnterpriseAttachEntity.class.getName());
        // 根据 u_id 获取 u_name
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据变更的对象，还没有进行比对，更新前")
    ...
    public SDataChangeLogDetailVo getDataChangeVoByUpdateBefore(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        // 查询更新前的数据
        MEnterpriseAttachEntity condition = (MEnterpriseAttachEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        MEnterpriseAttachEntity entity = enterpriseAttachMapper.selectById(condition.getId());
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据变更的对象，还没有进行比对，更新后")
    public SDataChangeLogDetailVo getDataChangeVoByUpdateAfter(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        MEnterpriseAttachEntity entity = (MEnterpriseAttachEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        vo.setOrder_code(enterpriseMapper.selectId(entity.getEnterprise_id()).getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
        vo.setResult(JSON.toJSONString(entity, JSONWriter.Feature.LargeObject));
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据删除的对象，还没有进行比对")
    ...
    public SDataChangeLogDetailVo getDataChangeVoByDelete(BoundSql boundSql) {
        ...
        MEnterpriseAttachEntity entity = enterpriseAttachMapper.selectById(id);
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        vo.setOrder_code(enterpriseMapper.selectId(entity.getEnterprise_id()).getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
        vo.setResult(JSON.toJSONString(entity, JSONWriter.Feature.LargeObject));
    ...
    public String getOrderCode(Integer id) {
        ...
        String code = enterpriseMapper.selectId(id).getCode();
    ...
    public SLogDataChangeDetailVo getCTimeExtension(String param, String _json_data, String clm_name, String clm_label) {
        ...
        if (!Objects.isNull(_json_data)) {
            ...
            try {
                MEnterpriseAttachEntity entity = JSONObject.parseObject(_json_data, MEnterpriseAttachEntity.class);
                localDateTime = enterpriseAttachMapper.selectById(entity.getId()).getC_time();
            } catch (Exception e) {
    ...
    public SLogDataChangeDetailVo getUTimeExtension(String param, String _json_data, String clm_name, String clm_label) {
        ...
        if (!Objects.isNull(_json_data)) {
            ...
            try {
                MEnterpriseAttachEntity entity = JSONObject.parseObject(_json_data, MEnterpriseAttachEntity.class);
                localDateTime = enterpriseAttachMapper.selectById(entity.getId()).getU_time();
            } catch (Exception e) {
