// filepath: d:\2025_project\10_all_project_develop\09_scm\02_后端\src\scm\scm-core\src\main\java\com\xinyirun\scm\core\system\serviceimpl\log\datachange\business\pocontract\DataChangeStrategyBPoContractDetailEntityServiceImpl.java
...
@Slf4j
@Service
public class DataChangeStrategyBPoContractDetailEntityServiceImpl extends LogChangeBaseServiceImpl implements IDataChangeStrategyService {
    ...
    @Autowired
    BPoContractDetailMapper detailMapper;
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据新增的对象，还没有进行比对")
    public SDataChangeLogDetailVo getDataChangeVoByInsert(BoundSql boundSql) {
        ...
        BPoContractDetailEntity entity = (BPoContractDetailEntity)parameterObject;
        ...
        vo.setEntity_name(BPoContractDetailEntity.class.getName());
        // 根据 u_id 获取 u_name
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据变更的对象，还没有进行比对，更新前")
    ...
    public SDataChangeLogDetailVo getDataChangeVoByUpdateBefore(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        // 查询更新前的数据
        BPoContractDetailEntity condition = (BPoContractDetailEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        BPoContractDetailEntity entity = detailMapper.selectById(condition.getId());
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoContractEntity masterEntity = poContractMapper.selectById(entity.getPo_contract_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据变更的对象，还没有进行比对，更新后")
    public SDataChangeLogDetailVo getDataChangeVoByUpdateAfter(BoundSql boundSql) {
        Object parameterObject = boundSql.getParameterObject();
        BPoContractDetailEntity entity = (BPoContractDetailEntity)((MapperMethod.ParamMap) parameterObject).get("param1");
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoContractEntity masterEntity = poContractMapper.selectById(entity.getPo_contract_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
...
//    @SysLogAnnotion("数据变更日志反射调用：获取数据删除的对象，还没有进行比对")
    ...
    public SDataChangeLogDetailVo getDataChangeVoByDelete(BoundSql boundSql) {
        ...
        BPoContractDetailEntity entity = detailMapper.selectByPoContractId(id);
        SDataChangeLogDetailVo vo = new SDataChangeLogDetailVo();
        BPoContractEntity masterEntity = poContractMapper.selectById(entity.getPo_contract_id());
        vo.setOrder_code(masterEntity.getCode());
        vo.setClass_name(this.getClass().getCanonicalName());
    ...
    public String getOrderCode(Integer id) {
        log.debug("------开始 获取 detailMapper code -----------");
        Integer masterId = detailMapper.selectById(id).getPo_contract_id();
        ...
        log.debug("------结束 获取 detailMapper code -----------");
        return code;
    }
    ...
    public SLogDataChangeDetailVo getCTimeExtension(String param, String _json_data, String clm_name, String clm_label) {
        ...
        if (!Objects.isNull(_json_data)) {
            ...
            try {
                BPoContractDetailEntity entity = JSONObject.parseObject(_json_data, BPoContractDetailEntity.class);
                BPoContractEntity masterEntity = poContractMapper.selectById(entity.getPo_contract_id());
