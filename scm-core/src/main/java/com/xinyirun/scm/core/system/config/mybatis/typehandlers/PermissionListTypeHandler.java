...
/**
 * 权限列表类型处理器
 * 用于处理MStaffVo中permissionList字段的JSON序列化和反序列化
 * 
 * @Author: zxh
 * @date 2025-08-23
 */
...
public class PermissionListTypeHandler extends BaseTypeHandler<List<PermissionItem>> {
     ...
     * @return 权限列表
     */
    private List<PermissionItem> parsePermissionList(String jsonContent) {
        ...
        if (StringUtils.isNotBlank(jsonContent)) {
            try {
                ...
                // 记录异常但不抛出，返回空列表
                System.err.println("解析权限列表JSON失败: " + jsonContent + ", 错误: " + e.getMessage());
            }
        }
