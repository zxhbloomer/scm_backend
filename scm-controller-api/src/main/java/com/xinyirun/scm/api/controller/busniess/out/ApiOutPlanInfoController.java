...
// @Api(tags = "添加出库计划")
@RestController
@RequestMapping(value = "/api/service/v1/outplan")
public class ApiOutPlanInfoController extends SystemBaseController {

    @Autowired
    private ApiIOutService service;

    @Autowired
    private IBOutPlanService iBoutService;
    ...
    @ResponseBody
    public ResponseEntity<ApiJsonResultAo<String>> discontinue(@RequestBody String code, HttpServletRequest request){
        try {
            List<BOutPlanListVo> beans = service.discontinue(code);
            // 调用外部接口，同步数据，注入当前Bean使得调用内部方法也被SpringAOP拦截
