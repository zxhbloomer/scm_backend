...
public interface AiUserQuotaMapper extends BaseMapper<AiUserQuotaEntity> {
     ...
     */
    @Insert("<script>" +
            "INSERT INTO ai_user_quota (id, user_id, daily_limit, monthly_limit, daily_used, monthly_used, " +
            "daily_reset_date, monthly_reset_date, total_cost, status, c_time, u_time, c_id, u_id, dbversion) " +
            "VALUES " +
            "<foreach collection='list' item='item' separator=','>" +
            "(#{item.id}, #{item.user_id}, #{item.daily_limit}, #{item.monthly_limit}, #{item.daily_used}, " +
            "#{item.monthly_used}, #{item.daily_reset_date}, #{item.monthly_reset_date}, #{item.total_cost}, #{item.status}, #{item.c_time}, #{item.u_time}, #{item.c_id}, #{item.u_id}, #{item.dbversion})" +
            "</foreach>" +
            "</script>")
     ...
     * 根据用户ID和租户查询配额
     */
    @Select("SELECT id, user_id, quota_type, total_quota, used_quota, remaining_quota, " +
    ...
    @Select("SELECT id, user_id, quota_type, total_quota, used_quota, remaining_quota, " +
            "reset_period, last_reset_time, is_active, create_time, update_time " +
            "FROM ai_user_quota " +
            "WHERE user_id = #{userId} AND quota_type = #{quotaType} AND is_active = 1 " +
            "LIMIT 1")
    ...
    @Select("SELECT * FROM ai_user_quota " +
            "WHERE user_id = #{userId} AND is_active = 1 LIMIT 1")
    AiUserQuotaEntity selectByUserIdAndTenant(@Param("userId") String userId);

    /**
     * 调用存储过程更新用户Token使用量
     */
    @Select("{CALL UpdateUserTokenUsage(#{userId}, #{tokenCount}, #{cost})}")
