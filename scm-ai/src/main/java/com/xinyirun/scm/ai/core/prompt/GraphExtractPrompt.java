package com.xinyirun.scm.ai.core.prompt;
...
public class GraphExtractPrompt {
    public static final String GRAPH_EXTRACTION_PROMPT = """
            ...
            Text: {input_text}
            ######################
            Output:""".replace("{tuple_delimiter}", AiConstant.GRAPH_TUPLE_DELIMITER)
            .replace("{entity_types}", Arrays.stream(AiConstant.GRAPH_ENTITY_TYPES).collect(Collectors.joining(",")))
            .replace("{completion_delimiter}", AiConstant.GRAPH_COMPLETION_DELIMITER)
            .replace("{record_delimiter}", AiConstant.GRAPH_RECORD_DELIMITER);
    ...
    public static final String GRAPH_EXTRACTION_PROMPT_CN = """
            ...
            ######################
            输出:
            """.replace("{tuple_delimiter}", AiConstant.GRAPH_TUPLE_DELIMITER)
            .replace("{entity_types}", Arrays.stream(AiConstant.GRAPH_ENTITY_TYPES).collect(Collectors.joining(",")))
            .replace("{completion_delimiter}", AiConstant.GRAPH_COMPLETION_DELIMITER)
            .replace("{record_delimiter}", AiConstant.GRAPH_RECORD_DELIMITER);
