...
public class SSEEmitterHelper {
    ...
    // 参考 aideepin: SSEEmitterHelper.java 第37行
     ...
     * 参考 aideepin: SSEEmitterHelper.checkOrComplete() 第39-55行
     *
     * @param userId 用户ID
    ...
    public boolean checkOrComplete(Long userId, SseEmitter sseEmitter) {
        ...
        // 1. 检查是否正在请求(并发控制)
        // 参考 aideepin: SSEEmitterHelper.java 第48-53行
        ...
        // 2. 检查请求次数限制(限流)
        // 参考 aideepin: SSEEmitterHelper.java 第41-45行 + RateLimitHelper.checkRequestTimes()
    ...
    /**
     * 开始SSE流式传输,设置Redis标记
     * 参考 aideepin: SSEEmitterHelper.startSse() 第57-59行
     *
     * @param userId 用户ID
     * @param sseEmitter SSE发射器
     */
     ...
     * 参考 aideepin: SSEEmitterHelper.startSse() 第61-80行
     *
     * @param userId 用户ID
     * @param sseEmitter SSE发射器
     * @param data 携带的数据（可选）
    ...
    public void startSse(Long userId, SseEmitter sseEmitter, String data) {
        ...
        // 设置正在请求标记,30分钟过期
        // 参考 aideepin: SSEEmitterHelper.java 第63-64行
        String askingKey = String.format(USER_ASKING_KEY, userIdStr);
        redisUtil.set(askingKey, "1", 30 * 60);  // 30分钟 = 1800秒

        // 增加请求计数
        // 参考 aideepin: SSEEmitterHelper.java 第66-67行 + RateLimitHelper.increaseRequestTimes()
        String countKey = String.format(USER_REQUEST_COUNT_KEY, userIdStr);
        ...
        // 发送START事件，可携带数据
        // 参考 aideepin: SSEEmitterHelper.java 第68-79行
        try {
    ...
    /**
     * 完成SSE,清理Redis状态
     * 参考 aideepin: SSEEmitterHelper.sendComplete() 第160-175行
     *
     * @param userId 用户ID
     * @param sseEmitter SSE发射器
     */
    public void sendComplete(Long userId, SseEmitter sseEmitter) {
        // 检查是否已完成
        // 参考 aideepin: SSEEmitterHelper.java 第161-165行
        ...
        try {
            // 发送DONE事件
            // 参考 aideepin: SSEEmitterHelper.java 第167-168行
            ...
            // 标记为已完成并清理Redis
            // 参考 aideepin: SSEEmitterHelper.java 第172-173行
            COMPLETED_SSE.put(sseEmitter, Boolean.TRUE);
            delSseRequesting(userId);
        }
    }
    ...
    /**
     * 发送错误信息并完成SSE
     * 参考 aideepin: SSEEmitterHelper.sendErrorAndComplete() 第190-206行
     *
     * @param userId 用户ID
     * @param sseEmitter SSE发射器
     * @param errorMsg 错误消息
    ...
    public void sendErrorAndComplete(Long userId, SseEmitter sseEmitter, String errorMsg) {
        // 检查是否已完成
        // 参考 aideepin: SSEEmitterHelper.java 第191-195行
        if (Boolean.TRUE.equals(COMPLETED_SSE.getIfPresent(sseEmitter))) {
        ...
        try {
            // 发送ERROR事件
            // 参考 aideepin: SSEEmitterHelper.java 第197行
            ...
            // 标记为已完成并清理Redis
            // 参考 aideepin: SSEEmitterHelper.java 第202-204行
            COMPLETED_SSE.put(sseEmitter, Boolean.TRUE);
            delSseRequesting(userId);
            sseEmitter.complete();
        }
    ...
    /**
     * 删除用户请求标记
     * 参考 aideepin: SSEEmitterHelper.delSseRequesting() 第208-211行
     *
     * @param userId 用户ID
     */
