/**
 * 基于JDBC的聊天记忆仓库抽象类，实现Spring AI的ChatMemoryRepository接口
 */
...
public abstract class JdbcChatMemoryRepository implements ChatMemoryRepository {
	...
	public static final String TABLE_NAME = "scm_ai_chat_memory";

	private static final String QUERY_GET_IDS = """
			SELECT DISTINCT conversation_id FROM scm_ai_chat_memory
			""";

	private static final String QUERY_ADD = """
			INSERT INTO scm_ai_chat_memory (conversation_id, content, message_type, timestamp) VALUES (?, ?, ?, ?)
			""";

	private static final String QUERY_GET = """
			SELECT content, message_type FROM scm_ai_chat_memory WHERE conversation_id = ? ORDER BY timestamp
			""";

	private static final String QUERY_CLEAR = "DELETE FROM scm_ai_chat_memory WHERE conversation_id = ?";
