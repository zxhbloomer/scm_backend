...
public interface BaseWorkerNodeMapper extends BaseMapper<WorkerNodeEntity> {
     ...
     */
    @Insert("<script>" +
            "INSERT INTO worker_node (host_name, port, type, launch_date, tenant, c_time, u_time, c_id, u_id, dbversion) " +
            "VALUES " +
    ...
    @Update("UPDATE worker_node " +
            "SET u_time = #{uTime}, u_id = #{uId}, dbversion = dbversion + 1 " +
            "WHERE id = #{id}")
     ...
     */
    @Update("UPDATE worker_node " +
            "SET launch_date = #{launchDate}, u_time = #{uTime}, u_id = #{uId}, dbversion = dbversion + 1 " +
            "WHERE id = #{id}")
