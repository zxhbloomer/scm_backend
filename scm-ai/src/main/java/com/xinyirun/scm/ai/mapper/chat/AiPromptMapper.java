...
public interface AiPromptMapper extends BaseMapper<AiPromptEntity> {
     ...
     */
    @Select("<script>" +
            "SELECT id, code, nickname, `desc`, type, prompt, " +
     ...
     * 查询所有有效提示词，按类型和名称排序
     */
    @Select("SELECT id, code, nickname, `desc`, type, prompt, " +
            "c_time, u_time, c_id, u_id, dbversion " +
            "FROM ai_prompt " +
     ...
     */
    @Insert("<script>" +
            "INSERT INTO ai_prompt (id, code, nickname, `desc`, type, prompt, " +
            "c_time, u_time, c_id, u_id, dbversion) VALUES " +
            "<foreach collection='list' item='item' separator=','>" +
            "(#{item.id}, #{item.code}, #{item.nickname}, #{item.desc}, #{item.type}, #{item.prompt}, " +
