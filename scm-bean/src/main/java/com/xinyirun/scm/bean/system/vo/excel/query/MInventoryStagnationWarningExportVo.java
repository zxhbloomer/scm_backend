...
@NoArgsConstructor
public class MInventoryStagnationWarningExportVo implements Serializable {
    ...
    @ExcelProperty(value="锁定库存", index = 14)
    private BigDecimal qty_lock;

    @ExcelProperty(value="库存单位", index = 15)
    private String unit_name;

    @ExcelProperty(value="实时单价", index = 16)
    private BigDecimal price;

    @ExcelProperty(value="货值", index = 17)
    private BigDecimal amount;

    @ExcelProperty(value="可用库存", index = 18)
    private BigDecimal qty_avaible;

    @ExcelProperty(value="最后一次入库时间", index = 19)
    @ColumnWidth(20)
    @DateTimeFormat("yyyy年MM月dd日")
    private LocalDateTime in_time;

    @ExcelProperty(value="最后一次出库时间", index = 20)
    @ColumnWidth(20)
    @DateTimeFormat("yyyy年MM月dd日")
    private LocalDateTime out_time;

    @ExcelProperty(value="最后一次调整时间", index = 21)
    @ColumnWidth(20)
    @DateTimeFormat("yyyy年MM月dd日")
