package com.xinyirun.scm.mongodb.serviceimpl.monitor.v1;
...
import com.xinyirun.scm.mongodb.repository.v1.MonitorMongoRepository;
...
@Service
public class MonitorDataMongoServiceImpl implements IMonitorDataMongoService {

    @Resource
    private MonitorMongoRepository repository;
     ...
     */
    @Override
    public List<BMonitorDataMongoEntity> saveAll(List<BMonitorDataMongoVo> userSaveParam) {
     ...
     */
    @Override
    public org.springframework.data.domain.Page<BMonitorBackupVo> selectPageList(BMonitorBackupVo param) {
        // 查询条件
        Criteria criteria = new Criteria();
        // 拼接模糊查询参数
        ...
        List<BMonitorBackupVo> resultList = BeanUtilsSupport.copyProperties(list, BMonitorBackupVo.class);
        org.springframework.data.domain.Page<BMonitorBackupVo> page = PageableExecutionUtils.getPage(resultList, pageParam, () -> count);
        return page;
    }
     ...
     */
    @Override
    public BMonitorBackupSumVo selectSumData(BMonitorBackupVo param) {
        // 查询条件
        Criteria criteria = new Criteria();
        // 拼接模糊查询参数
        concatParam(criteria, param);
        ...
        // 查询结果
        AggregationResults<BMonitorBackupSumVo> results = mongoTemplate.aggregate(Aggregation.newAggregation(BMonitorDataMongoEntity.class,
                match, group), BMonitorBackupSumVo.class);
     ...
     */
    @Override
    public BMonitorBackupDetailVo getDetail(BMonitorBackupVo param) {
        Query query = new Query();
        ...
        BMonitorBackupDetailVo result = (BMonitorBackupDetailVo) BeanUtilsSupport.copyProperties(vo.getDetailVo(), BMonitorBackupDetailVo.class);
        return result;
    }
     ...
     */
    @Override
    public List<BMonitorFileDownloadMongoVo> exportFile(List<BMonitorBackupVo> searchCondition) {
        // 获取ID
        Set<String> ids = searchCondition.stream().map(BMonitorBackupVo::getId).collect(Collectors.toSet());
        // 根据 ID 查询
        Criteria criteria = Criteria.where("id").in(ids);
        // 查询字段
        Query query = Query.query(criteria);
        query.fields().include("detailVo", "c_time", "vehicle_no", "code");
        List<BMonitorDataMongoEntity> entities = mongoTemplate.find(query, BMonitorDataMongoEntity.class);
        List<BMonitorFileDownloadMongoVo> result = new ArrayList<>();
        entities.forEach(item -> {
            List<BMonitorFileDownloadMongoVo> list = getBMonitorFileVo(item.getDetailVo().getPreviewFiles(),
     ...
     */
    @Override
    public List<BMonitorMongoExportVo> selectExportList(List<BMonitorBackupVo> searchCondition) {
        // 获取ID
        Set<String> ids = searchCondition.stream().map(BMonitorBackupVo::getId).collect(Collectors.toSet());
        // 根据 ID 查询
        Criteria criteria = Criteria.where("id").in(ids);
        ...
        List<BMonitorDataMongoEntity> entities = mongoTemplate.find(query, BMonitorDataMongoEntity.class);
        List<BMonitorMongoExportVo> list = BeanUtilsSupport.copyProperties(entities, BMonitorMongoExportVo.class);
        return list;
    }
     ...
     */
    @Override
    public List<BMonitorMongoExportVo> selectExportAllList(BMonitorBackupVo param) {
        // 查询条件
        Criteria criteria = new Criteria();
        // 拼接模糊查询参数
        concatParam(criteria, param);
     ...
     * 拼接查询参数
     */
    private void concatParam(Criteria criteria, BMonitorBackupVo param) {
        // 只查询没有恢复的数据
        criteria.and("is_restore").is(DictConstant.DICT_B_MONITOR_MONGO_IS_RESTORE_F);
        // 查询可见的数据
    ...
    private List<BMonitorFileDownloadMongoVo> getBMonitorFileVo(List<BPreviewBackupDataVo> previewFiles, LocalDateTime c_time, String vehicle_no, String code, String id) {
        List<BMonitorFileDownloadMongoVo> list = new ArrayList<>();
        ...
        for (BPreviewBackupDataVo vo : previewFiles) {
            BMonitorFileDownloadMongoVo bMonitorFileVo;
            switch (vo.getFile_num()) {
                case 1:
                    // 空车过磅 start
                    if (StringUtils.isNotEmpty(vo.getUrl())) {
                        // 空车过磅-司机车头照片
                        bMonitorFileVo = new BMonitorFileDownloadMongoVo();
                ...
                case 38:
                    if (StringUtils.isNotEmpty(vo.getUrl())) {
                        ...
                        BMonitorOutDeliveryDataMongoVo fileVo = getDetailFileVo(id);
...
//            bMonitorFileVo.setUrl(getPath(vo.getFile_18().getUrl().replaceAll("\\\\","/")));
        ...
        if (list.size() == 0) {
            BMonitorFileDownloadMongoVo bMonitorFileVo = new BMonitorFileDownloadMongoVo();
            bMonitorFileVo.setDirName(dirName);
            list.add(bMonitorFileVo);
        }
        return list;
    ...
    private void insertPreviewFiles(List<BPreviewBackupDataVo> previewFiles) {
        Set<Integer> nums = previewFiles.stream().map(BPreviewBackupDataVo::getFile_num).collect(Collectors.toSet());
        if (!nums.contains(38)) {
            previewFiles.add(new BPreviewBackupDataVo(38));
        }
        if (!nums.contains(39)) {
            previewFiles.add(new BPreviewBackupDataVo(39));
        }
    }
     ...
     * @return
     */
    private BMonitorOutDeliveryDataMongoVo getDetailFileVo(String id) {
        BMonitorBackupVo param = new BMonitorBackupVo();
        param.setId(id);
        BMonitorBackupDetailVo detail = this.getDetail(param);
