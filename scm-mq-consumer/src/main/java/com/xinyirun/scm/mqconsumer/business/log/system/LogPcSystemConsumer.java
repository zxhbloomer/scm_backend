package com.xinyirun.scm.mqconsumer.business.log.system;
...
//    private LogPcSystemMongoService mongoService;
    ...
    @RabbitListener(
        bindings = @QueueBinding(
            value = @Queue(value = MQEnum.MqInfo.LOG_PC_SYSTEM_QUEUE.queueCode, durable = "true"),
            exchange = @Exchange(name=MQEnum.MqInfo.LOG_PC_SYSTEM_QUEUE.exchange, durable = "true", type = "topic"),
            key = MQEnum.MqInfo.LOG_PC_SYSTEM_QUEUE.routing_key
        )
    )
    @RabbitHandler
    public void onMessage(@Payload Message messageDataObject, @Headers Map<String, Object> headers, Channel channel) throws IOException {
        ...
        log.debug("------消费者消费：start,message_id {}----- ", message_id);
...
//            logEntity.setMq_data(JSONObject.toJSONString(messageContext));
            ...
            log.debug("------消费者消费：end, message_id-----{}", message_id);
        }
    }
